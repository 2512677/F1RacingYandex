...
public class RCC_UI_DashboardDisplay : RCC_Core
{
    ...
    private void LateUpdate()
    {
        ...
        //––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
        //  Находим машину ТОЛЬКО игрока по тегу playerTag
        //––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
        if (findPlayerVehicleAuto)
        {
            // если текущая ссылка отсутствует или не имеет тег игрока — ищем по тегу
            if (!vehicle || !vehicle.CompareTag(playerTag))
            {
                GameObject playerObj = GameObject.FindGameObjectWithTag(playerTag);
                if (playerObj)
                    vehicle = playerObj.GetComponent<RCC_CarControllerV4>();
            }
        }
        ...
        if (ABS)
            ABS.color = Inputs.ABS ? color_On : color_Off;

        if (ESP)
            ESP.color = Inputs.ESP ? color_On : color_Off;

        if (Park)
            Park.color = Inputs.Park ? Color.red : color_Off;

        if (Headlights)
            Headlights.color = Inputs.Headlights ? Color.green : color_Off;
