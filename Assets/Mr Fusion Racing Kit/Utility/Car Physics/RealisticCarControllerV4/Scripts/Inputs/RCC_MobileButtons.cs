...
public class RCC_MobileButtons : RCC_Core {
    ...
    private void SetGyroLayout() {
        ...
        // Check if the current device supports an accelerometer 
        // (most mobile devices do, but this is false on many desktops).
        if (SystemInfo.supportsAccelerometer) {
            // Same idea: read Input.acceleration.x, then lerp it for smoothing.
            gyroInput = Mathf.Lerp(
                gyroInput,
                Input.acceleration.x * Settings.gyroSensitivity,
                Time.deltaTime * 5f
            );
        } else {
            gyroInput = 0f;
