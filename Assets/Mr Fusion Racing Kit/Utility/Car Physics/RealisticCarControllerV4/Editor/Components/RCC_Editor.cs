...
public class RCC_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        // Здесь можно отрисовать стандартный инспектор или вашу кастомную отрисовку
        DrawDefaultInspector();

        EditorGUILayout.Space();
        EditorGUILayout.LabelField("Импорт / Экспорт параметров автомобиля (JSON)", EditorStyles.boldLabel);
        EditorGUILayout.Space();

        if (GUILayout.Button("Экспорт CFG параметров"))
        {
            ExportParametersToCFG(carScript);
        }

        EditorGUILayout.Space();

        if (GUILayout.Button("Импорт CFG параметров"))
        {
            ImportParametersFromCFG(carScript);
        }
    ...
    // Экспорт параметров в CFG
    private void ExportParametersToCFG(RCC_CarControllerV4 car)
    {
        var p = CollectParameters(car);
        string path = EditorUtility.SaveFilePanel("Экспорт параметров в CFG", Application.dataPath, "RCCCarParams.cfg", "cfg");
        if (string.IsNullOrEmpty(path)) return;

        var sb = new StringBuilder();
        sb.AppendLine("# RCC Car Parameters Configuration");
        sb.AppendLine("// Можно добавлять комментарии, строки, начинающиеся с '#' или '//' будут игнорироваться");
        sb.AppendLine($"maxEngineTorque={p.maxEngineTorque}");
        sb.AppendLine($"maxEngineTorqueAtRPM={p.maxEngineTorqueAtRPM}");
        sb.AppendLine($"minEngineRPM={p.minEngineRPM}");
        sb.AppendLine($"maxEngineRPM={p.maxEngineRPM}");
        sb.AppendLine($"brakeTorque={p.brakeTorque}");
        sb.AppendLine($"maxspeed={p.maxspeed}");
        sb.AppendLine($"steerAngle={p.steerAngle}");
        sb.AppendLine($"highspeedsteerAngle={p.highspeedsteerAngle}");
        sb.AppendLine($"antiRollFrontHorizontal={p.antiRollFrontHorizontal}");
        sb.AppendLine($"antiRollRearHorizontal={p.antiRollRearHorizontal}");
        sb.AppendLine($"totalGears={p.totalGears}");
        sb.AppendLine($"gearShiftingDelay={p.gearShiftingDelay}");
        sb.AppendLine($"gearShiftingThreshold={p.gearShiftingThreshold}");
        sb.AppendLine($"gearShiftUpRPM={p.gearShiftUpRPM}");
        sb.AppendLine($"gearShiftDownRPM={p.gearShiftDownRPM}");
        sb.AppendLine($"finalRatio={p.finalRatio}");
        sb.AppendLine($"clutchInertia={p.clutchInertia}");
        sb.AppendLine($"engineInertia={p.engineInertia}");
        sb.AppendLine($"downForce={p.downForce}");
        sb.AppendLine($"limitMaxSpeed={p.limitMaxSpeed}");
        sb.AppendLine($"limitMaxSpeedAt={p.limitMaxSpeedAt}");
        sb.AppendLine($"useRevLimiter={p.useRevLimiter}");
        sb.AppendLine($"useNOS={p.useNOS}");
        sb.AppendLine($"useTurbo={p.useTurbo}");
        sb.AppendLine($"useExhaustFlame={p.useExhaustFlame}");
        File.WriteAllText(path, sb.ToString());
        EditorUtility.DisplayDialog("Экспорт CFG", "Настройки автомобиля успешно экспортированы!", "OK");
    }

    // Импорт параметров из JSON
    private void ImportParametersFromCFG(RCC_CarControllerV4 car)
    {
        string path = EditorUtility.OpenFilePanel("Импорт параметров из CFG", Application.dataPath, "cfg");
        if (string.IsNullOrEmpty(path)) return;

        var lines = File.ReadAllLines(path);
        var p = new RCCCarParameters();
        foreach (var raw in lines)
        {
            ...
            switch (key)
            {
                case "maxEngineTorque": p.maxEngineTorque = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "maxEngineTorqueAtRPM": p.maxEngineTorqueAtRPM = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "minEngineRPM": p.minEngineRPM = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "maxEngineRPM": p.maxEngineRPM = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "brakeTorque": p.brakeTorque = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "maxspeed": p.maxspeed = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "steerAngle": p.steerAngle = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "highspeedsteerAngle": p.highspeedsteerAngle = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "antiRollFrontHorizontal": p.antiRollFrontHorizontal = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "antiRollRearHorizontal": p.antiRollRearHorizontal = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "totalGears": p.totalGears = int.Parse(val); break;
                case "gearShiftingDelay": p.gearShiftingDelay = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "gearShiftingThreshold": p.gearShiftingThreshold = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "gearShiftUpRPM": p.gearShiftUpRPM = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "gearShiftDownRPM": p.gearShiftDownRPM = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "finalRatio": p.finalRatio = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "clutchInertia": p.clutchInertia = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "engineInertia": p.engineInertia = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "downForce": p.downForce = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "limitMaxSpeed": p.limitMaxSpeed = bool.Parse(val); break;
                case "limitMaxSpeedAt": p.limitMaxSpeedAt = float.Parse(val, System.Globalization.CultureInfo.InvariantCulture); break;
                case "useRevLimiter": p.useRevLimiter = bool.Parse(val); break;
                case "useNOS": p.useNOS = bool.Parse(val); break;
                case "useTurbo": p.useTurbo = bool.Parse(val); break;
                case "useExhaustFlame": p.useExhaustFlame = bool.Parse(val); break;
            }
        }
        ApplyParameters(car, p);
        EditorUtility.SetDirty(car);
        EditorSceneManager.MarkSceneDirty(car.gameObject.scene);
        EditorUtility.DisplayDialog("Импорт CFG", "Настройки автомобиля успешно импортированы!", "OK");
    }

    // Сбор параметров из текущего объекта автомобиля
    // --- Сбор параметров из текущего объекта автомобиля ---
    private RCCCarParameters CollectParameters(RCC_CarControllerV4 car)
    {
        RCCCarParameters p = new RCCCarParameters();
        p.maxEngineTorque = car.maxEngineTorque;
        p.maxEngineTorqueAtRPM = car.maxEngineTorqueAtRPM;
        p.minEngineRPM = car.minEngineRPM;
        p.maxEngineRPM = car.maxEngineRPM;
        p.brakeTorque = car.brakeTorque;
        p.maxspeed = car.maxspeed;
        p.steerAngle = car.steerAngle;
        p.highspeedsteerAngle = car.highspeedsteerAngle;
        p.antiRollFrontHorizontal = car.antiRollFrontHorizontal;
        p.antiRollRearHorizontal = car.antiRollRearHorizontal;
        p.totalGears = car.totalGears;
        p.gearShiftingDelay = car.gearShiftingDelay;
        p.gearShiftingThreshold = car.gearShiftingThreshold;
        // --- новые поля ---
        p.gearShiftUpRPM = car.gearShiftUpRPM;
        p.gearShiftDownRPM = car.gearShiftDownRPM;
        p.finalRatio = car.finalRatio;
        p.clutchInertia = car.clutchInertia;
        p.engineInertia = car.engineInertia;
        p.downForce = car.downForce;
        p.limitMaxSpeed = car.limitMaxSpeed;
        p.limitMaxSpeedAt = car.limitMaxSpeedAt;
        p.useRevLimiter = car.useRevLimiter;
        p.useNOS = car.useNOS;
        p.useTurbo = car.useTurbo;
        p.useExhaustFlame = car.useExhaustFlame;
        return p;
    }


    // Применение импортированных параметров к объекту автомобиля
    // --- Применение импортированных параметров к объекту автомобиля ---
    private void ApplyParameters(RCC_CarControllerV4 car, RCCCarParameters p)
    {
        car.maxEngineTorque = p.maxEngineTorque;
        car.maxEngineTorqueAtRPM = p.maxEngineTorqueAtRPM;
        car.minEngineRPM = p.minEngineRPM;
        car.maxEngineRPM = p.maxEngineRPM;
        car.brakeTorque = p.brakeTorque;
        car.maxspeed = p.maxspeed;
        car.steerAngle = p.steerAngle;
        car.highspeedsteerAngle = p.highspeedsteerAngle;
        car.antiRollFrontHorizontal = p.antiRollFrontHorizontal;
        car.antiRollRearHorizontal = p.antiRollRearHorizontal;
        car.totalGears = p.totalGears;
        car.gearShiftingDelay = p.gearShiftingDelay;
        car.gearShiftingThreshold = p.gearShiftingThreshold;
        // --- новые поля ---
        car.gearShiftUpRPM = p.gearShiftUpRPM;
        car.gearShiftDownRPM = p.gearShiftDownRPM;
        car.finalRatio = p.finalRatio;
        car.clutchInertia = p.clutchInertia;
        car.engineInertia = p.engineInertia;
        car.downForce = p.downForce;
        car.limitMaxSpeed = p.limitMaxSpeed;
        car.limitMaxSpeedAt = p.limitMaxSpeedAt;
        car.useRevLimiter = p.useRevLimiter;
        car.useNOS = p.useNOS;
        car.useTurbo = p.useTurbo;
        car.useExhaustFlame = p.useExhaustFlame;
        // Пересоздаём кривую крутящего момента
        car.ReCreateEngineTorqueCurve();
    }

}

// --- Расширенный класс для сериализации всех параметров ---
[System.Serializable]
public class RCCCarParameters
{
    public float maxEngineTorque;
    public float maxEngineTorqueAtRPM;
